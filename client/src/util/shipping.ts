/**
 * Shipping engine — the CLIENT MIRROR of server/src/lib/shipping.ts.
 *
 * Everything from the first declaration below is IDENTICAL to the server file, and
 * `npm run verify` compares the two byte for byte from that point. Change one, change
 * both — the pack drawer, the container fit bars and the ship guards must refuse exactly
 * what the API is about to refuse, or the UI promises something the server then rejects.
 *
 * (This header deliberately does not quote the marker declaration: verify.ts finds it with
 * indexOf, so a mention up here would match before the real thing.)
 *
 * The same arrangement as costing.ts, expr.ts and pricing.ts.
 */
