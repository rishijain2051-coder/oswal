/**
 * Client-side mirror of server/src/lib/pricing.ts — MUST stay identical.
 *
 * Used for live totals as a proforma or order is edited: the subtotal, the charges, the
 * CGST/SGST/IGST split and the grand total all update as you type, and must match what
 * the API will compute to the paisa. Mirrored the same way costing.ts and expr.ts are.
 *
 * The rules, in the order they apply:
 *
 * 1. A LINE is qty x unitPrice, less its discount percentage, less its flat discount.
 * 2. A CHARGE belongs to the whole document and carries its own GST rate; a percentage
 *    charge is a percentage of the line subtotal only, so charges never compound.
 * 3. TAX applies per rate. Intra-state splits half CGST / half SGST; inter-state is all
 *    IGST. Which one is DERIVED by comparing the buyer's state with the company's.
 * 4. An OVERSEAS document is zero-rated end to end — every rate on it is ignored.
 */
